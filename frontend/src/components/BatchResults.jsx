...
import { lookupCnpj } from '../utils/api'
import { formatCnpj } from '../utils/cnpj'
import jsPDF from 'jspdf'
import autoTable from 'jspdf-autotable'

export default function BatchResults({ cnpjs, invalidInputs, onBack }) {
  ...
  // New state to filter by PJ Partners
      ...
      for (let i = 0; i < cnpjs.length; i++) {
        ...
        try {
          ...
          setResults(prev => [...prev, {
            ...
            socios: data.company.qsa.map(s => `${s.nome || s.nome_socio} (${s.qual || s.qualificacao_socio}) [${s.tipo}]`),
  ...
  // Combine and filter items based on toggle
  const filteredItems = useMemo(() => {
    const all = [
      ...results,
      ...invalidInputs.map(inv => ({
        cnpj: inv, status: 'Inválido', razao_social: '-', invalid: true, has_pj_partners: false
      }))
    ]
    if (showOnlyPj) {
  ...
  return (
    <div className="glass-panel" style={{ width: '100%', maxWidth: '900px', margin: '0 auto', padding: 'var(--space-xl)', display: 'flex', flexDirection: 'column', gap: 'var(--space-lg)' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-subtle)', paddingBottom: 'var(--space-md)' }}>
        <div>
          <h2 style={{ fontSize: 'var(--font-size-2xl)', fontWeight: '600', color: 'var(--accent-gold)', marginBottom: '0.25rem' }}>
            Processamento em Lote
          </h2>
          <p style={{ color: 'var(--text-secondary)', fontSize: 'var(--font-size-sm)' }}>
            Análise automática de múltiplas empresas
          </p>
        </div>
        <Button variant="secondary" onClick={() => { setIsCanceled(true); onBack(); }} style={{ height: 'fit-content' }}>
          Voltar
        </Button>
      </div>
      ...
      {isProcessing && (
        <div style={{ background: 'var(--bg-surface-elevated)', padding: 'var(--space-lg)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-strong)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 'var(--space-sm)' }}>
            <span style={{ fontSize: 'var(--font-size-lg)', fontWeight: '500' }}>
              Progresso
            </span>
            <span style={{ fontSize: 'var(--font-size-xl)', fontWeight: 'bold', color: 'var(--accent-gold)' }}>
              {progress}%
            </span>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 'var(--space-sm)', color: 'var(--text-secondary)', fontSize: 'var(--font-size-sm)' }}>
            ...
            <span>{currentIndex} de {cnpjs.length} processados</span>
          </div>
        </div>
      )}
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', background: 'var(--bg-surface-elevated)', padding: '0.5rem 1rem', borderRadius: '50px', border: '1px solid var(--border-strong)' }}>
            <input 
              type="checkbox" 
              checked={showOnlyPj} 
              onChange={(e) => setShowOnlyPj(e.target.checked)} 
          ...
          <div style={{ display: 'flex', gap: '1rem' }}>
            <Button variant="primary" onClick={handleExportCSV}>Exportar CSV</Button>
                ...
                <React.Fragment key={idx}>
                  ...
                  >
                    <td style={{ padding: '1rem', textAlign: 'center', color: 'var(--text-muted)' }}>
                      {!r.invalid && (
                        <span style={{ transform: isExpanded ? 'rotate(90deg)' : 'rotate(0deg)', display: 'inline-block', transition: 'transform 0.2s ease' }}>
                          ▶
                        </span>
                      )}
                    </td>
                    <td style={{ padding: '1rem', fontFamily: 'monospace', fontSize: 'var(--font-size-sm)' }}>
                      {r.invalid ? r.cnpj : formatCnpj(r.cnpj)}
                    </td>
                    <td style={{ padding: '1rem' }}>
                      {r.has_pj_partners 
                        ? <span style={{ display: 'inline-block', padding: '0.25rem 0.5rem', background: 'rgba(184, 155, 100, 0.15)', color: 'var(--accent-gold)', borderRadius: '4px', fontSize: '0.75rem', fontWeight: '600' }}>TEM SÓCIO PJ</span> 
                        : <span style={{ display: 'inline-block', padding: '0.25rem 0.5rem', background: 'rgba(210, 211, 213, 0.1)', color: 'var(--text-muted)', borderRadius: '4px', fontSize: '0.75rem', fontWeight: '600' }}>SEM PJ</span>
                      }
                    </td>
                                ...
                                {r.socios.map((s, i) => {
                                  ...
                                  return (
                                    <div key={i} style={{ 
                                      ...
                                      {isPj && <span style={{ marginLeft: 'auto', fontSize: '0.7rem', padding: '0.1rem 0.4rem', background: 'var(--accent-gold)', color: '#000', borderRadius: '4px', fontWeight: 'bold' }}>PJ</span>}
                                    </div>
                                  )
                                })}
                              </div>
