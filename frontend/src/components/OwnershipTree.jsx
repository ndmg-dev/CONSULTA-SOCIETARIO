...
function OwnershipNode({ node, depth = 0, isRoot = false }) {
  ...
  return (
    <div className={`ownership-node ${nodeTypeClass} ${rootClass}`}>
      <div className="ownership-node__header" onClick={hasChildren ? handleToggle : undefined}>
        ...
        <span className="ownership-node__name">
          {node.nome || node.name || node.razao_social || '—'}
        </span>

        {isPJ && cleanedCnpj.length === 14 && (
          <span
            className="ownership-node__cnpj"
            onClick={handleNavigate}
            style={{ cursor: 'pointer' }}
            title="Consultar este CNPJ"
          >
        ...
        {node.qualificacao && (
          <span className="ownership-node__qual">{node.qualificacao}</span>
        )}

        {isPJ && (
          <StatusBadge variant="gold">PJ</StatusBadge>
        )}

        {!isPJ && !isCycle && !isUnresolved && (
          <StatusBadge variant="silver">PF</StatusBadge>
        )}

        {isCycle && (
          <span className="ownership-node__cycle-badge">
            ⟲ Ciclo detectado
          </span>
        )}

        {isUnresolved && (
          <span className="ownership-node__unresolved-badge">
            ⚠ Não resolvido
          </span>
        )}

        {depth > 0 && (
          <span className="ownership-node__depth">Nível {depth}</span>
        )}
      </div>
...
export default function OwnershipTree({ tree }) {
  if (!tree) return null
  ...
  return (
    <div className="ownership-tree-container">
      <OwnershipNode node={rootNode} depth={0} isRoot />
